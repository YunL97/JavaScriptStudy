...
user.getName(); 
```
* 화살표 함수에서는 call, apply, bind 등등 많은 것들을 사용하지 못함
* 화살표 함수에서는 일반적인 함수에서 가지고 있는 것들을  다가지고 있지는 않음
...
* 유효성 검사를 프론트에서 해주면 서버와의 비용 절감이 가능 ex) 이메일 이상한거 들어갈떄
* 유효성 검사는 할수 있는 모든곳에서 다하는게 좋음
* 클라이언트를 개발하는 입장에서 사용자의 입력을 받는데 개발자가 모든 에러를 예측하여 처리하기가 어렵도 거의 불가능
* try catch 를 사용할때
* catch에는
  * 개발자를 위한 예외처리
...
```
* blackbox: 
  * 내부 구현이 어떻게 동작될지 예측할 수 없는경우 사용
  * 추상화가 너무 과하게 되거나 명시적인 코드가 아닌경우

```
button.addEventListener('click', (): void=> {

}) // 로 클릭하면 어떻게 되는지 정확하게 알 수 있음
```
* 공백도 코드작성의 일부다
1. 선언
2. 로직, 문
